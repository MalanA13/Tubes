...
// GetMyCourierShipments mengambil semua shipment yang ditugaskan ke courier yang sedang login.
//
// Flow:
//  1. Validasi claims
//  2. Ambil semua shipment yang CourierUserID-nya cocok dengan user yang login
//  3. Return list shipments
func (s *Service) GetMyCourierShipments(ctx context.Context, claims *domain.AuthClaims) ([]*domain.Shipment, error) {
	// 1. Validasi claims
	if claims == nil {
	...
	// 2. Ambil shipments dari repository
	shipments, err := s.repo.ListShipmentsByCourier(ctx, claims.UserID)
	if err != nil {
		return nil, fmt.Errorf("get-my-courier-shipments: gagal ambil shipments: %w", err)
	}
