...
// HandleListAllOrdersHTTP handles GET /admin/orders.
// Returns all orders in the system without user filtering (admin endpoint).
// Requires authentication with admin role (enforced by middleware).
func HandleListAllOrdersHTTP(service order.OrderService) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		orders, err := service.ListAllOrders(r.Context())
		if err != nil {
			response.InternalServerError(w, err.Error())
			return
		}
		...
		for i, o := range orders {
			result[i] = toOrderDetailResponse(o)
		}
