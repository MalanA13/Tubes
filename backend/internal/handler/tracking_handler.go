...
// HandleListAllShipmentsHTTP returns all shipments in the system for admin use.
// Requires authentication with admin role (enforced by middleware).
func HandleListAllShipmentsHTTP(service tracking.TrackingService) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		shipments, err := service.ListAllShipments()
		if err != nil {
			response.InternalServerError(w, err.Error())
			return
		}
