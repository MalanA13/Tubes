...
type TrackingRepository interface {
	...
	ListAllShipments() ([]ShipmentModel, error)
}
...
// ListAllShipments returns all shipments in the system, ordered by most recently updated first.
// Returns an empty slice (not nil) when no shipments exist.
func (r *trackingRepository) ListAllShipments() ([]ShipmentModel, error) {
	var shipments []ShipmentModel
	err := r.db.Order("updated_at DESC").Find(&shipments).Error
	if err != nil {
		return nil, err
	}
